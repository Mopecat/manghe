...
export default {
    ...
    getTotal() {
      ...
      if (this.list) {
        this.list.forEach((item) => {
          total += Math.floor(item.currprice - 1000)
        })
      }
      return total
    }
  ...
  methods: {
    ...
    getList(page) {
      list({ page, status: 3, type: 'salelist' })
        .then((data) => {
    ...
    exportExcel() {
      ...
      /* 从表生成工作簿对象 */
      var wb = XLSX.utils.table_to_book(document.querySelector('#out-table-shoukuan'), xlsxParam)
      /* 获取二进制字符串作为输出 */
